import { PhysicalRooms, VirtualRoom } from "@/app/type/rooms";


export function getAvailableTimes(room: PhysicalRooms, date: string): { roomId: number, availableTimes: { start: string, end: string }[] } {
...
export function getAvailableTimesVirtual(room: VirtualRoom, date: string): { roomId: number, availableTimes: { start: string, end: string }[] } {
    const reservations = room.reservation.filter(reserve => {
        const data1 = new Date(reserve.reserve_date).toISOString()
        const data2 = new Date(date + "T00:00:00.000Z").toISOString()
    ...
    return { roomId: room.virtual_room_id, availableTimes }
}
